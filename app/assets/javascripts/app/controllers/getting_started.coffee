      ...
      success: (data, status, xhr) =>
        if data.result is 'ok'
          ...
          if data.content_messages && data.content_messages > 0 && (!@account['inbound']['options'] || @account['inbound']['options']['keep_on_server'] isnt true)
            message = App.i18n.translateContent('We have already found %s email(s) in your mailbox. Zionsoftwares Helpdesk will move it all from your mailbox into Zionsoftwares Helpdesk.', data.content_messages)
    ...
    @ajax(
      ...
      success: (data, status, xhr) =>
        if data.result is 'ok'
          ...
          if data.content_messages && data.content_messages > 0 && (!@account['inbound']['options'] || @account['inbound']['options']['keep_on_server'] isnt true)
            message = App.i18n.translateContent('We have already found %s emails in your mailbox. Zionsoftwares Helpdesk will move it all from your mailbox into Zionsoftwares Helpdesk.', data.content_messages)
            @$('.js-inbound-acknowledge .js-message').html(message)
